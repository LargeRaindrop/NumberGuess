...
int main()
{
	system("title 猜数字小游戏V2.4 made by 小雨点儿");
	guess_the_number();
	return 0;
}
void guess_the_number()
...
void start_game()
{
	...
	for(times=1;;times++)
	{
		...
		{
			clock_end=clock();
			cout<<"恭喜你猜对了数字："<<std_num<<",一共猜了"<<times<<"次，用时"<<(db)(clock_end-clock_start)/1000<<"秒\n";
			break;
		}
	}
	if (get_answer)
	{
		clock_end=clock();
		cout<<"答案是："<<std_num<<"，一共猜了"<<times<<"次，用时"<<(db)(clock_end-clock_start)/1000<<"秒\n";
	} 
